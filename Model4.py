...
# Piston motion
    ...
    # Calculate flow rates based on presssure difference
    if x_v_arr[n] >= 0:
...
# Plot the difference in pressure between Chamber A and B
plt.figure(figsize=(12, 6))
plt.plot(time, p_WE_1_arr - p_WE_2_arr, label='Pressure Difference (simulation)', color='blue', linewidth=2)
plt.plot(time_data, p_WE_1_data - p_WE_2_data, label='Pressure Difference (data)', linestyle='dashed', color='red', linewidth=1)
plt.xlabel('Time [s]', fontname="Times New Roman", fontsize=12)
plt.ylabel('Pressure Difference [Pa]', fontname="Times New Roman", fontsize=12)
plt.title('Pressure Difference in Chambers A and B vs Time', fontname="Times New Roman", fontsize=14)
plt.legend(fontsize=10, loc='upper left')
plt.grid(color='gray', linestyle='--', linewidth=0.5)
plt.tick_params(axis='both', which='major', labelsize=10)
...
plt.plot(time, tau_cyl1_arr, label='Torque from Cylinder 1', color='green', linewidth=2)
plt.plot(time, tau_cyl2_arr, label='Torque from Cylinder 2', color='orange', linewidth=2)
