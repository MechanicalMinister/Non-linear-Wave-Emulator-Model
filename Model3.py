...
V_WE_1_0 = 0.01  # Initial volume in chamber A (m^3)
V_WE_2_0 = 0.01  # Initial volume in chamber B (m^3)
...
C_le = 1e-12  # Leakage coefficient
...
Ts = 1e-4  # Time step (s)
SimulationTime = 30 # Total simulation time (s)
numSteps = int(SimulationTime / Ts+1)  # Number of simulation steps

# Read data from Excel sheet
tout_data = pd.read_excel("C:/Users/Rober/OneDrive - Aalborg Universitet/MP.dok/Studie/Master/1. Semester/Project/Disable servo valve from 0.xlsx", sheet_name='tout', header=0)
yout_data = pd.read_excel("C:/Users/Rober/OneDrive - Aalborg Universitet/MP.dok/Studie/Master/1. Semester/Project/Disable servo valve from 0.xlsx", sheet_name='yout', header=0)
...
ddot_theta_data = np.gradient(dot_theta_data, time_data)    
...
ddot_theta_data = ddot_theta_data[start_row:end_row]
...
ddot_theta_data = np.interp(np.linspace(0, SimulationTime, numSteps), np.linspace(0, SimulationTime, len(ddot_theta_data)), ddot_theta_data)
...
Q_1_arr[0] = Q_n / np.sqrt(p_n) * x_v_arr[0] * np.sqrt(np.abs(p_S_val - p_WE_1_arr[0])) * ((p_S_val - p_WE_1_arr[0]) / np.abs(p_S_val - p_WE_1_arr[0]))
Q_2_arr[0] = Q_n / np.sqrt(p_n) * x_v_arr[0]*0.7 * np.sqrt(np.abs(p_WE_2_arr[0] - p_T_val)) * ((p_WE_2_arr[0] - p_T_val) / np.abs(p_WE_2_arr[0] - p_T_val))
...
max_ddot_theta = np.deg2rad(5)
min_ddot_theta = np.deg2rad(-5)
# Dynamic model
...
for n in tqdm(range(1, numSteps), desc="Simulation Progress"):    
    ...
    if x_v_arr[n] >= 0:
        Q_1_arr[n] = Q_n / np.sqrt(p_n) * x_v_arr[n] * np.sqrt(np.abs(p_S_val - p_WE_1_arr[n-1])) * ((p_S_val - p_WE_1_arr[n-1]) / np.abs(p_S_val - p_WE_1_arr[n-1]))
    else:
        Q_1_arr[n] = Q_n / np.sqrt(p_n) * x_v_arr[n] * np.sqrt(np.abs(p_WE_1_arr[n-1] - p_T_val)) * ((p_WE_1_arr[n-1] - p_T_val) / np.abs(p_WE_1_arr[n-1] - p_T_val))
    
    if x_v_arr[n] >= 0:
        Q_2_arr[n] = Q_n / np.sqrt(p_n) * x_v_arr[n]*0.7 * np.sqrt(np.abs(p_WE_2_arr[n-1] - p_T_val)) * ((p_WE_2_arr[n-1] - p_T_val) / np.abs(p_WE_2_arr[n-1] - p_T_val))
    else:
        Q_2_arr[n] = Q_n / np.sqrt(p_n) * x_v_arr[n]*0.7 * np.sqrt(np.abs(p_S_val - p_WE_2_arr[n-1])) * ((p_S_val - p_WE_2_arr[n-1]) / np.abs(p_S_val - p_WE_2_arr[n-1]))
    ...
    dot_p_WE_1_arr[n] = (beta_1_arr[n]/10) / (V_WE_1_0 + A_WE_1 * (x_WE_arr[n-1])) * (Q_1_arr[n] - C_le * (p_WE_1_arr[n-1] - p_WE_2_arr[n-1]) - A_WE_1 * dot_x_WE_arr[n-1])
    dot_p_WE_2_arr[n] = (beta_2_arr[n]/10) / (V_WE_2_0 - A_WE_2 * (L_WE-x_WE_arr[n-1])) * (C_le * (p_WE_1_arr[n-1] - p_WE_2_arr[n-1]) - Q_2_arr[n] + A_WE_2 * dot_x_WE_arr[n-1])
    ...
    F_WE_arr[n] = A_WE_1 * p_WE_1_arr[n-1] - A_WE_2 * p_WE_2_arr[n-1]
    ...
    # Ensure ddot_theta is within the range of -100 to 100 degrees/s^2
...
plt.plot(time, x_v_arr, label='Valve Input (simulation)', color='blue', linewidth=2)
